...
// clean version up top, other comments are previous tries.
function acceptomatic(){
...
/////////////////////////////////////////////////////////
//code here is valid, but no return just commented for usability
...
function rejectomatic (){
    return new Promise((resolve, reject) => {
        setTimeout(() =>{
            reject(Error("Too slow"));
        }, 5000);
    }).catch((whateverTheRejectErrorMessageIs) => {
        console.log(`is it really ${whateverTheRejectErrorMessageIs}`);
    })
}
rejectomatic();
...
function chainOfPromises(){
    return new Promise((resolve, reject) =>{
        resolve(numToSum+=5);
        console.log(`top ${numToSum}`);
    }).then(addFive => {
        return new Promise((resolve, reject) =>{
        resolve(addFive += 5);
        console.log(`mid ${addFive}`);})
    })
    .then(addFive => {
        return new Promise((resolve, reject) =>{
        resolve (addFive +=5);
        console.log(`bottom ${addFive}`);})
    });
